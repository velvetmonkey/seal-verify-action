...
async function run({ env = process.env, cwd = process.cwd(), stdout = process.stdout } = {}) {
  const setEmptyOutputs = () => {
    for (const [name, value] of [["verified", 0], ["reduced_scope", 0], ["failed", 0],
  ...
  for (const r of results) {
    ...
    gh.error(stdout, r.detail || r.status, {
      file: r.relPath,
      title: r.status === "reduced-scope"
        ? "Seal receipt at reduced scope — not independently verified"
        : "Seal receipt not verified",
    });
  }

  const { verified, reducedScope, failed } = summarize(results);
  gh.setOutput(env, "verified", verified);
  gh.setOutput(env, "reduced_scope", reducedScope);
  gh.setOutput(env, "failed", failed);
  const { signatureValid, replayConsistent, replayScope, authorityTrusted } = aggregateVerdicts(results);
  ...
  gh.log(
    stdout,
    `seal-verify: ${verified} verified, ${failed} failed (${reducedScope} at reduced scope) (verifier ${pin.KIT_NAME} ${pin.KIT_VERSION} @ ${pin.KIT_COMMIT.slice(0, 7)})`
  );

  // Three distinct exits: 0 VERIFIED, 4 REDUCED SCOPE (all shortfalls are
  // §11.1 authorised-unparseable — non-passing but not a hard failure), 3
  // UNPINNED, 1 hard failure. `failed` includes reducedScope, so a mix of
  // reduced-scope and a hard failure still exits 1 (fail closed).
  if (failed === 0) return 0;
  if (failed === reducedScope) return 4;
  return authorityTrusted === "unpinned" ? 3 : 1;
}

// Fold per-receipt results into the fleet verdicts. EVERY aggregate is guarded
// against vacuous truth: `[].every()` is `true`, so a set with nothing in scope
// would otherwise CLAIM signature/replay/authority consistency having verified
// NOTHING. The `length > 0` prefix on each aggregate is that guard — a zero-size
// set yields `false`, never a spurious "all good over nothing". kernel_replay_scope
// discloses coverage (applicable/matched) alongside the replay boolean. §11.1
// unparseable-request receipts carry no (tool, arguments) and are OUT of replay
// scope (replay_applicable === false), not replay failures. Pinned directly in
// test/main.test.js so a future edit that drops a length guard goes RED.
function aggregateVerdicts(results) {
  ...
  const authorityTrusted = results.length > 0 && results.every((r) => r.authority_trusted === true)
    ...
    : results.length > 0 && results.every((r) => r.verificationCore === true && r.authority_trusted === "unpinned")
      ? "unpinned" : false;
  return { signatureValid, replayConsistent, replayScope, authorityTrusted };
}

module.exports = { run, aggregateVerdicts };
