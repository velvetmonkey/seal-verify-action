...
function toMarkdown(results, { pin, verifierVersion, patterns, workingDirectory }) {
  ...
  for (const r of results) {
    lines.push(
      `| \`${mdEscape(r.relPath)}\` | ${STATUS_LABEL[r.status] || r.status} | ${r.signature_valid === true} | ${r.kernel_replay_consistent === true} | ${mdEscape(r.authority_trusted ?? false)} | ${mdEscape(r.detail || "")} |`
    );
  }
  lines.push("");
